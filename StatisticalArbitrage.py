...
from statsmodels.tsa.vector_ar.vecm import select_coint_rank as johansen
from statsmodels.tsa.vector_ar.vecm import VECM
import copy as cp



### Titolo: Statistical Arbitrage
...
class PairsTrading(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        RollingWindowSize = 30  # La rolling window ha estensione 30
        self.Data = {}
        ...
        self.maxPositions = 2  # scegliamo un numero massimo di posizioni distinte da avere in portafoglio in un qualsiasi momento (in questa sede, con posizione si fa riferimento ad un gruppetto distinto di elementi cointegrati, quindi può significare comprare/vendere diverse azioni distinte)
    ...
    def OnData(self, data):
        ...
        if self.Xcompleted and la.norm(self.X, 2)>10:   # se la matrice è aggiornata e non zero
            ...
            if self.FirstTime or self.RebuildSubGroups %30 == 0:
                ...
                ## -Si esegue kmeans su X1 per ottenere sottogruppi piccoli (5-6 elementi), su cui nella terza fase si conduranno i test
                ...
                for k in range(0, self.kclusters):
                    ...
                    num = int(np.ceil(numberOfElements / 5))  # costruiamo sottogruppi con mediamente 5-6 elementi: per cui, in maniera più immediata, si devono effettuare pochi test per sottogruppo (se vengono soddisfatti si considerano candidati validi, altrimenti si controllano gli elementi del gruppo successivo)
                    ...
                    for kk in range(0, num):  # fase di scelta dei gruppi su cui operare effettuando gli opportuni test
                        ...
                        if numSubGroup < 3:  # non abbiamo abbastanza elementi per continuare con l'analisi
                            continue
                        else:  # siamo nel caso in cui il cluster abbia almeno 3 elementi (sufficienti per tentare di trovare un sottogruppo)
                            ...
                            ## -Si effettua il test di Johansen tra tale elemento ed un altro del sottogruppo: si verifica che il sistema costituito da una coppia di azioni abbia matrice di rango 1; altrimenti si riprova con un altro titolo del sottogruppo
                            ## -Nel caso in cui sia stata evidenziata cointegrazione nella coppia di elementi, si intende ora accertarsi che il sistema costituito dalla coppia e da un altro elemento del sottogruppo sia cointegrato con matrice di rango 1 (e si continua così fino a costruire un sistema di 5-6 elementi, se possibile)
                            ## -I sottogruppi individuati vengono raccolti in un'istanza TradeableGroup e successivamente memorizzati in un dizionario
                            
                            
                            symbDifferent=[symb for symb in symbSubGroup if symb != symbCentroid] # cerchiamo relazioni di cointegrazione dentro un sottogruppo
                            XX=cp.copy(self.Data[symbCentroid].PriceSeries)
                            resultSymb=[]
                            resultSymb.append(symbCentroid)
                            for symb in symbDifferent:
                                Xprec=cp.copy(XX)
                                XX = np.column_stack((XX,  self.Data[symb].PriceSeries))
                                j=johansen(XX, -1, 1, method='trace', signif=0.01)  # no termini costanti ; no trend;
                                if j.rank==1:   # se il rango del test è 1
                                    resultSymb.append(symb)   # aggiungiamo il simbolo al sottogruppo
                                    if len(resultSymb)==4:   # abbiamo già un sottogruppo di 4 elementi : stop
                                        break
                                
                                else:   # riproviamo con un altro elemento
                                    XX=cp.copy(Xprec)
                                    
               
                            
                            if len(resultSymb) > 2:
                                vecm=VECM(XX, exog=None, exog_coint=None, dates=None, freq=None, missing='none', k_ar_diff=1, coint_rank=1, deterministic='nc', seasons=0, first_season=0)
                                param=vecm.fit()
                                beta=param.beta   # coefficienti che identificano l'attrattore, ovvero la combinazione lineare generata
                                

                                
                                
                                self.myTradableGroup.Mu = 0
                                totsum=0
                                for k in range(0, len(beta)):
                                    totsum += beta[k] * np.array(self.Data[resultSymb[k]].PriceSeries)
                                
                                
                                self.myTradableGroup.Res = totsum - self.myTradableGroup.Mu  # valore attuale della combinazione lineare
                                self.myTradableGroup.Sigma = np.std(self.myTradableGroup.Res)
                                self.myTradableGroup.SymbolTradableElements = resultSymb
                ...
                for k in range(0, self.kclusters):
                    ...
                    for j in range(0, len_subGroups):
                        if ClusterDictionary[k].SubGroups.get(j) == None:  # non abbiamo un sottogruppo per tale indice j
                            ...
                            dictionarySorted[i] = ClusterDictionary[k].SubGroups[j].Score
            ...
            if len(self.PositionManagement.OpenPositions) != self.maxPositions:  # deteniamo al massimo maxPositions=2 sottogruppi in portafoglio: quindi se in un determinato momento ne abbiamo 2, allora il portafoglio è pieno e non facciamo nulla
                if len(self.PositionManagement.ReadySubGroups) >= 1:
                    ...
                    for val in list(valSet):
                        if len(self.PositionManagement.OpenPositions) == self.maxPositions:  # abbiamo già riempito tutte le 2 posizioni prestabilite
                            break
